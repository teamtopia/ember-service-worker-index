...
module.exports = class Config extends Plugin {
  ...
  build() {
    let options = this.options; // options from 'esw-index' config
    ...
    let timeout =
      isNaN(configuredTimeout) || configuredTimeout <= 0
        ? 500
        : configuredTimeout; // 0 would be the same as cache-first strategy
